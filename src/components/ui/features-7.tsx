  ...
  return (
    ...
    >
      <button
        ...
        className={cn(
          "group relative w-full text-left p-6 rounded-xl border transition-all duration-300 hover:shadow-md bg-white overflow-hidden",
          isActive 
            ? "border-transparent shadow-xl ring-1 ring-gray-400/10" 
            : "border-gray-200 bg-gray-50/50 hover:bg-white hover:border-gray-300"
        )}
      >
        {/* Internal magic border - only when active */}
        {isActive && (
          <div className="absolute inset-[3px] rounded-[9px] pointer-events-none">
            <div className="absolute inset-0 rounded-[9px] magic-border" />
          </div>
        )}
