...
// Compact, centered overlay on the active tab (countdown)
const CenteredOverlay: React.FC<{ left: number; top: number; progress: number }> = ({
  left,
  top,
  progress,
}) => {
...
const Features7: React.FC<{ className?: string }> = ({ className }) => {
  ...
  // 10s auto-cycle progress (0..1)
  const [progress, setProgress] = React.useState(0);
  const rafRef = React.useRef<number | null>(null);
  const timeoutRef = React.useRef<number | null>(null);
  const startRef = React.useRef<number>(0);

  // Absolute position (relative to container) for centered overlay
  ...
  // Position the overlay in the center of the active tab
  const updateIndicatorPos = React.useCallback(() => {
    ...
    if (!activeTab) {
      ...
      activeTab = chipId ? (root.querySelector(`#chip-tab-${chipId}`) as HTMLElement | null) : null;
    }
  ...
  // Restart cycle and reposition overlay on active change
  React.useEffect(() => {
    startCycle();
  ...
  return (
    <section className={cn("w-full py-24 sm:py-28 relative overflow-hidden", className)}>
      ...
      <div ref={containerRef} className="max-w-[1216px] mx-auto px-6 relative">
        ...
        {/* Centered countdown overlay (does not affect layout) */}
        {indicatorPos ? (
          <CenteredOverlay left={indicatorPos.left} top={indicatorPos.top} progress={progress} />
        ) : null}

        {/* Layout: Image first, Content second (restored) */}
        <div className="mt-12 grid grid-cols-1 lg:grid-cols-2 gap-10">
          {/* Visual content (restored order with dynamic border) */}
          ...
          {/* Textual content (restored to the right) */}
          <div>
