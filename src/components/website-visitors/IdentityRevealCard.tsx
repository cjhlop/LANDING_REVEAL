...
const IdentityRevealCard = ({ active }: { active: boolean }) => {
  ...
  return (
    <div className="relative group perspective-1000 z-50">
      {/* Card Container - Wider (w-96) */}
      <div className={cn(
        "w-96 bg-white rounded-2xl shadow-[0_20px_50px_-12px_rgba(0,0,0,0.15)] border border-gray-100 overflow-hidden transition-all duration-700 ease-out transform",
        active ? "translate-y-0 opacity-100 rotate-x-0" : "translate-y-8 opacity-0 rotate-x-6"
      )}>
        
        {/* Premium Header Background - Slightly shorter (h-24) */}
        ...
        <div className="px-6 pb-5 relative">
          ...
          {/* Avatar - Floating & Animated - Slightly smaller (w-20) */}
          <div className="absolute -top-10 left-6">
            <div className={cn(
              "w-20 h-20 rounded-2xl bg-white p-1.5 shadow-xl transition-all duration-700 cubic-bezier(0.34, 1.56, 0.64, 1)",
              ...
              <div className="w-full h-full rounded-xl bg-gray-100 overflow-hidden relative group/avatar">
                ...
                />
                {/* Online Indicator */}
                <div className="absolute bottom-0 right-0 w-3.5 h-3.5 bg-green-500 border-[2px] border-white rounded-full shadow-sm"></div>
              </div>
            </div>
          </div>
          ...
          {/* Name & Role Section - Reduced margins */}
          ...
          {/* Contact Details Grid - Tighter spacing */}
          ...
          {/* Bottom: Match Score Indicator - Reduced margin */}
          <div className={cn(
            "mt-4 p-2.5 rounded-xl bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-100 flex items-center justify-between transition-all duration-700 delay-700 shadow-sm",
...
const ContactRow = ({ icon: Icon, label, value, delay, visible, highlight }: any) => (
  <div 
    className={cn(
      "flex items-center justify-between group/row p-2 rounded-lg transition-all duration-500 border border-transparent",
  ...
  >
    <div className="flex items-center gap-3">
      ...
      <div className="flex flex-col">
        <span className="text-[9px] font-semibold text-gray-400 uppercase tracking-wider leading-tight">{label}</span>
