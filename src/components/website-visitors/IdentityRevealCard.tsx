...
const IdentityRevealCard = ({ active }: { active: boolean }) => {
  ...
  return (
    <div className="relative group perspective-1000 z-50">
      {/* Card Container - Kept w-96 */}
      <div className={cn(
        "w-96 bg-white rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-gray-100 overflow-hidden transition-all duration-700 ease-out transform",
        active ? "translate-y-0 opacity-100 rotate-x-0" : "translate-y-8 opacity-0 rotate-x-6"
      )}>
        
        {/* Premium Header - Deep Professional Blue */}
        ...
        <div className="px-6 pb-5 relative">
          ...
          {/* Top Right: Verification Badge - Minimalist */}
          <div className="flex justify-end pt-3">
            <div className={cn(
              "flex items-center gap-1.5 px-2 py-1 rounded-md bg-blue-50 text-blue-700 text-[10px] font-semibold uppercase tracking-wide transition-all duration-500 delay-300",
          ...
          {/* Contact Details Grid - Clean List */}
          ...
          {/* Bottom: Match Score - Professional Bar */}
          <div className={cn(
            ...
            <div className="flex flex-col">
              <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">ICP Match Score</span>
...
const ContactRow = ({ icon: Icon, label, value, delay, visible, highlight }: any) => (
  ...
  >
    <div className="flex items-center gap-3">
      ...
      <span className={cn("text-sm", highlight ? "font-medium text-gray-900" : "text-gray-600")}>{value}</span>
    </div>
